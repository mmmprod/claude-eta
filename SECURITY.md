...
| `projects/<project_fp>/completed/<session>__<agent>.jsonl` | Completed turns used for `/claude-eta:eta`, stats, insights, compare, and eval |
...
| `export/velocity-YYYY-MM.json` | Local anonymized export generated by `/claude-eta:eta export` |
...
## `/claude-eta:eta inspect`

`/claude-eta:eta inspect` shows:
...
- `/claude-eta:eta compare` is read-only. It fetches aggregate baselines and never uploads your local records.
- `/claude-eta:eta contribute` is blocked until you explicitly run `/claude-eta:eta community on`.
- `/claude-eta:eta contribute` without `--confirm` previews what would be sent.
- `/claude-eta:eta contribute --confirm` uploads only anonymized aggregate task records.

## What `/claude-eta:eta contribute` sends
...
`/claude-eta:eta compare` also keeps a local cache at `cache/baselines.json` for 6 hours.
