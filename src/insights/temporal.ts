...
export function sessionFatigue(tasks: CompletedTask[]): SessionFatigueResult | null {
  const sessions = groupBy(tasks, (t) => t.session_id);

  // Keep sessions with 3+ tasks, sort each by timestamp
  const qualifiedSessions: CompletedTask[][] = [];
  ...
  if (qualifiedSessions.length < 3) return null;

  // Collect durations by ordinal position + laterPositions in a single pass
  const byPosition = new Map<number, number[]>();
  const laterPositions: number[] = [];
  let pos1Durations: number[] | undefined;
  ...
  if (!pos1Durations || pos1Durations.length === 0 || laterPositions.length === 0) return null;

  const avgByPosition: SessionFatigueResult['avgByPosition'] = [];
...
export function timeOfDayPatterns(tasks: CompletedTask[]): TimeOfDayResult | null {
  if (tasks.length < 15) return null;

  const buckets = groupBy(tasks, (t) => getPeriod(new Date(t.timestamp_start).getHours()));

  const byPeriod: TimeOfDayResult['byPeriod'] = [];

  for (const period of ['morning', 'afternoon', 'evening', 'night'] as Period[]) {
...
export function weeklyTrends(tasks: CompletedTask[]): WeeklyTrendsResult | null {
  ...
  if (weekEntries.length < 4) return null;
  ...
  const direction: 'improving' | 'degrading' | 'stable' =
    changeRate < -10 ? 'improving' : changeRate > 10 ? 'degrading' : 'stable';
