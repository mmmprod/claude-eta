...
export function fileOperationRatios(tasks: CompletedTask[]): FileOpsResult | null {
  if (tasks.length < 5) return null;

  const groups = groupBy(tasks, (t) => t.classification);
  const byClassification: FileOpsResult['byClassification'] = [];
  ...
  if (byClassification.length === 0) return null;
...
export function efficiencyScoring(tasks: CompletedTask[]): EfficiencyResult | null {
  const valid = tasks.filter((t) => t.tool_calls > 0);
  if (valid.length < 5) return null;

  const groups = groupBy(valid, (t) => t.classification);
  const byClassification: EfficiencyResult['byClassification'] = [];
