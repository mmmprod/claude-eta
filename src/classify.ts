...
const PATTERNS: [TaskClassification, RegExp][] = [
  ['bugfix', /\b(fix(e[ds]|ing)?|bugs?|issues?|errors?|broken|crash(e[ds]|ing)?|wrong|fail(ed|ing|ure|s)?|patch(e[ds]|ing)?|hotfix|regression)\b/i],
  ...
  ['refactor', /\b(refactor(ing|ed)?|rename[ds]?|extract(ing|ed)?|move[ds]?|split|merge[ds]?|clean(ing|ed|up)?|simplif(y|ied|ying)|reorganize[ds]?|restructure[ds]?)\b/i],
  ['docs', /\b(docs?|readme|comments?|jsdoc|typedoc|changelog|documentation)\b/i],
  ['config', /\b(config(ure)?|setup|install(ing|ed|s)?|deps?|dependenc(y|ies)|eslint|prettier|tsconfig|package\.json|ci|cd|pipeline|docker|env)\b/i],
  ['review', /\b(review(ing|ed|s)?|pr|pull.?request|audit(ing|ed)?|check(ing)?|inspect(ing)?|look.?at)\b/i],
  ['feature', /\b(add(ing|ed|s)?|creat(e[ds]?|ing)|implement(ing|ed|s)?|build(ing|s)?|new|feature|component|page|endpoint|api|hook|integrat(e[ds]?|ing))\b/i],
];
