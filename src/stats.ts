 ...
 */
export const INITIAL_PRIORS: Record<TaskClassification, { low: number; median: number; high: number }> = {
...
export function computeStats(tasks: AnalyticsTask[]): ProjectStats | null {
  ...
  for (const task of tasks) {
    if (task.duration_seconds == null || !Number.isFinite(task.duration_seconds) || task.duration_seconds <= 0) continue;
