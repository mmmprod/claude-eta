...
interface ContributeState {
  ...
  contributed_work_item_ids: string[];
  /** @deprecated Kept for backward-compat reading of old state files. */
...
function parseState(raw: Partial<ContributeState>): ContributeState {
  const contributedWorkItemIds = Array.isArray(raw.contributed_work_item_ids)
    ? raw.contributed_work_item_ids.filter((id): id is string => typeof id === 'string')
    : [];
...
function saveState(count: number, newWorkItemIds: string[]): void {
...
function getNewRecords(cwd: string, pluginVersion: string): { records: AnonymizedRecord[]; taskIds: string[] } {
  ...
  for (const task of tasks) {
    if (excludeIds.has(task.analytics_id)) continue;
...
export async function showContribute(cwd: string, pluginVersion: string): Promise<void> {
  ...
  if (state && state.last_contributed_at) {
    console.log(
      `${c.dim('Last contribution:')} ${state.last_contributed_at} ${c.dim(`(${state.last_contributed_count} records)`)}`,
    );
