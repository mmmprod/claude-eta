...
export function getEtaCommandPath(homeDir = os.homedir()) {
    return path.join(homeDir, '.claude', 'commands', 'eta.md');
}
export function buildEtaCommandAlias(pluginRoot) {
...
export function isManagedEtaCommand(content) {
    return content.includes(ALIAS_MARKER);
}
export function ensureEtaCommandAlias(pluginRoot, homeDir = os.homedir()) {
