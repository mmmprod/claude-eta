...
function renderBreakdown(title, rows) {
    ...
    return [
        ...
        c.dim('|----------------------------|-----|------------------|------------------|------------------|'),
        ...rows.map((row) => `| ${c.bold(pad(row.key, 26))} | ${c.dim(pad(String(row.sample_count), 3))} | ${coverageColor(row.prompt.p80_coverage_pct)(pad(fmtMetric(row.prompt), 16))} | ${coverageColor(row.first_edit.p80_coverage_pct)(pad(fmtMetric(row.first_edit), 16))} | ${coverageColor(row.first_bash.p80_coverage_pct)(pad(fmtMetric(row.first_bash), 16))} |`),
        '',
    ];
}
export function formatEvaluationReport(report) {
    ...
    for (const stage of ['prompt', 'first_edit', 'first_bash']) {
        ...
        lines.push(`| ${c.bold(pad(stage, 11))} | ${c.dim(pad(samples, 7))} | ${c.cyan(pad(mdape, 5))} | ${coverageColor(report.overall[stage].p80_coverage_pct)(pad(coverage, 12))} |`);
    }
    lines.push('');
