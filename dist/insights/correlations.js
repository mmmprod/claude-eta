...
export function errorDurationCorrelation(tasks) {
    if (tasks.length < 10)
        return null;
    const withErrors = tasks.filter((t) => (t.errors ?? 0) > 0);
    const withoutErrors = tasks.filter((t) => (t.errors ?? 0) === 0);
    if (withErrors.length < 3 || withoutErrors.length < 3)
        return null;
...
export function contextSwitchCost(tasks) {
    if (tasks.length < 2)
        return null;
    ...
    if (totalPairs < 10 || sameDurations.length < 3 || diffDurations.length < 3)
        return null;
...
export function volatilityRootCauses(tasks) {
    ...
    if (!bestCls || bestTasks.length < 10)
        return null;
